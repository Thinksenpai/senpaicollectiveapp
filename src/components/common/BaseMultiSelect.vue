...
<template>
  <Listbox :model-value="modelValue" @update:model-value="handleChange" multiple :disabled="disabled">
    <div class="relative">
      <ListboxLabel v-if="label" class="block text-sm font-medium text-gray-700 mb-1">
        {{ label }}
        <span v-if="required" class="text-red-500">*</span>
        <span v-if="max" class="text-gray-400 font-normal"> (max {{ max }})</span>
      </ListboxLabel>
      <ListboxButton
        :class="[
          'relative w-full cursor-pointer rounded-lg border bg-white py-2 pl-3 pr-10 text-left focus:outline-none focus:ring-2 focus:ring-offset-0 sm:text-sm min-h-[42px]',
      ...
      >
        ...
        <span v-else class="flex flex-wrap gap-1">
          <span
            ...
            :key="option.value"
            class="inline-flex items-center gap-1 rounded bg-senpai-100 px-2 py-0.5 text-xs font-medium text-senpai-700"
          >
            {{ option.label }}
            <button
              type="button"
              @click.stop="removeOption(option.value)"
              class="hover:text-senpai-900"
            >
      ...
      >
        <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          <ListboxOption
            ...
            :key="option.value"
            :value="option.value"
            :disabled="option.disabled || (max !== undefined && modelValue.length >= max && !modelValue.includes(option.value))"
